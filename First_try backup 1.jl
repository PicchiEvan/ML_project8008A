...
using Plots, OpenML, DataFrames, CSV,StatsPlots ,MLJ, MLJLinearModels, Random

# ╔═╡ 1eca1647-32af-4d63-a131-12e3a3ba6d8a
md"## 1) Creation des set de données"

# ╔═╡ 2467bc6f-e824-4472-800f-b429e1c482de
begin
	...
	dropmissing!(training)
	#training_dr_m = dropmissing(training)
	#test_dr_miss = dropmissing(test)
	
end

# ╔═╡ b48847dd-8f85-4575-b703-fc511ea0bf85


# ╔═╡ 8a910fd2-d10d-42ef-8542-f52e596c0d1a

coerce!(training, :precipitation_nextday => Binary);


# ╔═╡ f99b015f-b7de-4219-ae8b-bf39db179b3b
#fonction de perte
...
# ╔═╡ d4ddcf35-8908-4c37-a14f-d6a13ab41f3c
...
# ╔═╡ 800a554c-c0e3-45ec-afb1-790773ad8529
begin
	split_d1=data_split(training;shuffle = true,idx_train = 1:1359,idx_valid=1:1,idx_test =1340:1699);
	#Sans set de validation
	train_data_nv = split_d1.train;
	test_data_nv = split_d1.test;
	
end

# ╔═╡ aecd6dc8-4474-41f5-bc4c-0c82e0ad118f
begin
	split_d2=data_split(training;shuffle = true,idx_train = 1:1359,idx_valid=1:1,idx_test =1340:1699);
	train_data_v=split_d2.train;
	validation_data_v =split_d2.valid;
	test_data_v=split_d2.test;
end

# ╔═╡ 997df7c8-e4fc-4cb4-8bd6-114835280d2a
md"## 2)Multiple logistic regression "

# ╔═╡ 259c5699-0138-4432-a6ef-9dee6a547a9c
md"1) sans ridge regression"
...
function tune_model(model, data)
	tuned_model = TunedModel(model = model,
	                         ...
	                         measure = auc)
	machine(tuned_model, select(data, Not(:precipitation_nextday)), data.precipitation_nextday) |> fit!
end
...
#fit!( machine(LogisticClassifier(penalty = :l1),
                #select(train_data_nv, Not(:precipitation_nextday)),
                #train_data_nv.precipitation_nextday));

#auc_L1 = [losses(fit!(machine(LogisticClassifier(penalty = :l1,lambda = l),
                #select(train_data_nv, Not(:precipitation_nextday)),
                #train_data_nv.precipitation_nextday))))]
...
# ╔═╡ 5d55f329-cdcd-4fb8-a1f9-a98d242ac2e7
report(mach_log_L1)

# ╔═╡ 96944a19-0f4d-429d-aefe-45e1bb4b3dde
#On teste avec L2
#mach_log_L2 = tune_model(LogisticClassifier(penalty=:l2),train_data_nv)

# ╔═╡ 57aff087-e2f3-4856-b724-e58d6eb381e3
md"## 3) polynomial"

# ╔═╡ acf1d1ff-e35e-4248-852a-ed7bf5e20423
md"1) On optimise juste le degré"

# ╔═╡ ef0f72ee-04e7-47d1-a69e-0e93f557d41a
begin
	self_tuning_deg_pol = TunedModel(model = model,
                                   resampling = CV(nfolds = 10),
                                   tuning = Grid(),
                                   range = range(model, :degree, values = 1:20),
                                   measure = auc)
    deg_pol_only_mach = fit!(machine(self_tuning_model, select(train_data_nv, Not(:precipitation_nextday)), train_data_nv.precipitation_nextday) )
end

# ╔═╡ 6853414f-401a-4b2e-ac25-29976daa2a3a
md"2) degré + L1 L2"

# ╔═╡ ccd6975d-3dd1-4494-93ff-6acc9797bd67
md"L1"

# ╔═╡ 7b479cc0-ff05-4cf1-b8ed-c03a38c1ecb8
begin
	
end

# ╔═╡ 2151e738-9c04-4219-8c05-60b8ecdf8347
...
# ╔═╡ 88927225-b876-4f53-9122-635373c0e482
md"## 5) PCA"

# ╔═╡ 0897e449-c922-49fd-a218-f6bb98a2e35c
